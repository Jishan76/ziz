...
module.exports = {
    ...
    onStart: async function ({ message, args }) {
        ...
        async function generateCoverImage(characterId) {
            ...
            const apiUrl = `https://nguyenmanh.name.vn/api/avtWibu?id=${characterId}&chunen=${encodeURIComponent(name)}&chuky=${encodeURIComponent(slogan)}&apikey=${apiKey}`;
            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    const tempFilePath = path.join(__dirname, `cover2_${Date.now()}.jpg`);
